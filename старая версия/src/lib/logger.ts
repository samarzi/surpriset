...
class Logger {
  ...
  private sanitizeData(data: any): any {
    ...
    // Список чувствительных ключей для маскировки
    const sensitiveKeys = [
      'password', 'token', 'apikey', 'secret', 'auth', 'authorization',
      'user_id', 'email', 'phone', 'first_name', 'last_name', 'username',
      'telegram_id', 'chat_id', 'session_id', 'access_token', 'refresh_token'
    ]
      ...
      for (const [key, value] of Object.entries(obj)) {
        if (sensitiveKeys.some(sensitive => key.toLowerCase().includes(sensitive))) {
          if (typeof value === 'string' && value.length > 0) {
            result[key] = value.length > 8 ? `${value.slice(0, 4)}***${value.slice(-2)}` : '***'
          } else {
            result[key] = '[REDACTED]'
          }
  ...
  private addLog(level: LogLevel, message: string, data?: any, source?: string): void {
    ...
    const logEntry: LogEntry = {
      ...
      level,
      message,
      data: this.sanitizeData(data), // Санитизируем данные
      source,
  ...
  private logToConsole(entry: LogEntry): void {
    // В продакшене выводим только WARN и ERROR
