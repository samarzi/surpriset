...
export function useSwipeBack(options: SwipeBackOptions = {}) {
  const {
    enabled = true,
    threshold = 60, // Еще меньше для более быстрого срабатывания
    velocity = 0.2, // Еще меньше минимальная скорость
  } = options;
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
      ...
      console.log('🏁 Swipe end - deltaX:', deltaX, 'velocity:', swipeVelocity);
